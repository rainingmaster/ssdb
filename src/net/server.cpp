...
static const int WRITER_THREADS = 1;  // 蹇呴』涓?, 鍥犱负鏌愪簺鍐欐搷浣滀緷璧栧崟绾跨▼
...
NetworkServer::NetworkServer(){
	...
	fdes = new Fdevents(); // 在 epoll 下建立 epoll_create
	...
	// init ip_filter
	{
		...
		if(cc != NULL){
			...
			for(it = children->begin(); it != children->end(); it++){
				if((*it)->key == "allow"){ //白名单 ip 过滤
					const char *ip = (*it)->str();
					log_info("    allow %s", ip);
					serv->ip_filter->add_allow(ip);
				}
				if((*it)->key == "deny"){ //黑名单 ip 过滤
					const char *ip = (*it)->str();
					log_info("    deny %s", ip);
...
void NetworkServer::serve(){
	...
	writer->start(num_writers); //开启 num_writers 个 writer 线程
	reader = new ProcWorkerPool("reader"); // WorkerPool<ProcWorker, ProcJob>::WorkerPool
	reader->start(num_readers); //开启 num_readers 个 reader 线程
	...
	while(!quit){
		...
		ready_list.swap(ready_list_2); // 释放内存
		ready_list_2.clear();
		...
		for(int i=0; i<(int)events->size(); i++){
			...
			if(fde->data.ptr == serv_link){ //响应的 fd 为 listen 的链接
				...
				if(proc_result(job, &ready_list) == PROC_ERROR){ // 处理弹出的 job
					//
				}
			}else{ // 断开链接时
		...
		for(it = ready_list.begin(); it != ready_list.end(); it ++){
			Link *link = *it;
			if(link->error()){ // 断开链接
				this->link_count --; // 减少链接数
				fdes->del(link->fd()); //去除改 fd 的监听
				delete link;
				continue;
			}
