...
NetworkServer* NetworkServer::init(const char *conf_file, int num_readers, int num_writers){
	...
	NetworkServer* serv = init(*conf, num_readers, num_writers); //µ÷ÓÃÏÂÃæµÄÖØÔØº¯Êý
	delete conf;
	return serv;
}
...
NetworkServer* NetworkServer::init(const Config &conf, int num_readers, int num_writers){
	...
	NetworkServer *serv = new NetworkServer(); //µ÷ÓÃ¹¹Ôìº¯Êý
	if(num_readers >= 0){
...
void NetworkServer::serve(){
	...
	writer->start(num_writers); //Æô¶¯¶à¸öÐ´Ïß³Ì
	...
	// ÉèÖÃÈý¸öÐèÒª¼àÌýµÄÎÄ¼þÃèÊö·û£¬serv_link¼àÌýÐÂÁ¬½Óµ½À´£¬reader¶ÁÈÎÎñÏß³Ì³Ø£¬writeÐ´ÈÎÎñÏß³Ì³Ø
	...
	while(!quit){
		...
		// Õë¶ÔÒÑ¾­ ready µÄ¶ÓÁÐ½øÐÐ´¦Àí
		for(it = ready_list.begin(); it != ready_list.end(); it ++){
			Link *link = *it;
			if(link->error()){ // Á´½ÓÓÐÎÊÌâ(»òÕß¶Ô·½Ö÷¶¯ close ÁË)£¬¶Ï¿ªÁ´½Ó
				this->link_count --;
				fdes->del(link->fd());
				delete link;
				continue;
