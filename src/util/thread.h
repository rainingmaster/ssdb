...
int WorkerPool<W, JOB>::start(int num_workers){
	...
	int err;
	pthread_t tid;
	for(int i=0; i<num_workers; i++){ //启动 num_workers 个线程
		struct run_arg *arg = new run_arg();
		arg->id = i;
		arg->tp = this;

        /* 使用线程执行 _run_worker，参数为上面复制的 arg */
		err = pthread_create(&tid, NULL, &WorkerPool::_run_worker, arg);
		if(err != 0){
			fprintf(stderr, "can't create thread: %s\n", strerror(err));
		}else{
			tids.push_back(tid); //创建成功，存入 tids 向量组
		}
	}
	started = true;
	return 0;
